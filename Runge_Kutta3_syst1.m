...
for i=1:N
    ...
    t(i+1)=t(i)+h;
    %update y1 & y2
    K1y1=fy1(t(i),y1(i),y2(i));
    K1y2=fy2(t(i),y1(i),y2(i));
    K2y1=fy1(t(i)+h/3,y1(i)+h/3*K1y1,y2(i)+h/3*K1y2);
    K2y2=fy2(t(i)+h/3,y1(i)+h/3*K1y1,y2(i)+h/3*K1y2);
    K3y1=fy1(t(i)+2*h/3,y1(i)+2*h/3*K2y1,y2(i)+2*h/3*K2y2);
    K3y2=fy2(t(i)+2*h/3,y1(i)+2*h/3*K2y1,y2(i)+2*h/3*K2y2);
    y1(i+1)=y1(i)+(h/4)*(K1y1+3*K3y1);
    y2(i+1)=y2(i)+(h/4)*(K1y2+3*K3y2);
end
Fy1=fexy1(t);
Fy2=fexy2(t);
errory1=abs(Fy1-y1);
...
hold on
plot(t,Fy1,'ko','linewidth',3)
%title('RK3 syst','fontsize',20)
...
hold on
plot(t,Fy2,'mo','linewidth',3)
%title('RK3 syst','fontsize',20)
xlabel('$t$','FontSize',20,'InterPreter','Latex')
ylabel('$y_2(t)$','FontSize',20,'InterPreter','Latex')
grid on
legend('Approximate Solution','Exact Solution')
set(gca,'fontsize',15.3)

figure(3)
plot(t,errory1,'r-','linewidth',3)
xlabel('$t$','FontSize',20,'InterPreter','Latex')
